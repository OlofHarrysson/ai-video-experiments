...
The final [coral-mask comparison](../experiments/krea-coral.md) has an [editable graph](krea-coral.json), [API graph](krea-coral.api.json) and required [input mask](krea-coral-band.png). Upload the mask with this filename to ComfyUI before running. Both branches use the same seed and native Krea eight-step settings. The regional branch combines a weak complete-scene prompt with complementary coral/stone masks. It tightens coverage without reliably completing the requested curve. The API executed and export schemas were checked; frontend opening was not repeated.

The [three-robot benchmark](../experiments/krea-layouts.md) provides two exact executed comparison graphs: layout A [editable](krea-layout-A.json) / [API](krea-layout-A.api.json), and layout B [editable](krea-layout-B.json) / [API](krea-layout-B.api.json). Each runs a complete prompt and masked regional prompts with the same seed and Krea eight-step settings, without refinement. The regional outputs have substantial prop/body failures; these are preserved experiment graphs, not recommended production recipes. Both API graphs executed and their editable exports were schema-checked; frontend opening was not repeated.
