...
Previous execution: [Klein 4B and Krea 2 Pod results](projects/modern-model-study/experiments/pod-results.md). Both produced their own openings and three-second cadence-3 twist clips. Krea keeps a steadier palette but redraws geometry and produces some cadence ghosting; Klein preserves layout more closely but accumulates harsh colors. All media is local and the Pod is deleted. Use the project's `audition.py` and direct `pod_client.py` for this recipe; the legacy `experiment.py` SDXL commands remain separate. Pods are authorized whenever they make experimentation more convenient. [Compatibility audit](../../docs/research/modern-model-transition.md) and [prompting guidance](../../docs/research/prompting-for-feedback.md) explain the recipe differences.
