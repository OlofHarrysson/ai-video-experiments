...
Prepared, blocked before inference: [Klein 4B and Krea 2 audition](projects/modern-model-study/experiments/baseline.md), approved with model-specific prompts and each model generating its own opening/feedback. No new-model media was generated; compute and temporary storage are cleaned up. Reuse the motion/review tools. The active worker prepares modern weights on local worker disk and archives outputs on a temporary network volume; the original SDXL recipe is `serverless/Dockerfile.sdxl`. Use the project's `audition.py` for modern models; the legacy `experiment.py` commands below expect SDXL/Difforum. [Compatibility audit](../../docs/research/modern-model-transition.md) and [prompting guidance](../../docs/research/prompting-for-feedback.md) explain the recipe differences.
