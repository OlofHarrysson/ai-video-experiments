...
The package migration has passed local replay and isolated wheel import checks; installation in the retained Pod runtime has not yet been performed. When copying a migrated runner, also install the matching `deforum_lab` package through uv into that runner's explicitly selected environment. Build locally with `uv build --out-dir work/package-refactor/dist` and transfer the wheel alongside the source. Confirm the existing NumPy, Pillow and OpenCV versions before installation; do not silently change the numerical libraries or the retained Torch/CUDA environment during an experiment.

For an environment whose declared dependencies have already been verified, `uv pip install --python /absolute/path/to/runner/python --no-deps /absolute/path/to/wheel.whl` installs only the package. Resolve those paths from the actual session. Then use that same Python to import `deforum_lab.rendering.feedback` and run the experiment's `--help` before submission. Keep ComfyUI's diagnostic nodes and the separate RIFE environment on their established paths. The extracted Pod path does not import `boto3`; some legacy runner chains still do, and the app dependency remains declared for them.
