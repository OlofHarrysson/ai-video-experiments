...
def graph(model, prompt, seed, *, editing=False, denoise=0.45):
    ...
    g = {
        "1": node(
            ...
            if klein
            else "krea2_turbo_fp8_scaled.safetensors",
        ...
        "2": node(
            ...
            if klein
            else "qwen3vl_4b_fp8_scaled.safetensors",
            type="flux2" if klein else "krea2",
            device="default",
        ),
        "3": node(
            "VAELoader",
            vae_name="flux2-vae.safetensors" if klein else "qwen_image_vae.safetensors",
        ),
        "4": node("CLIPTextEncode", clip=["2", 0], text=prompt),
        "5": node("ConditioningZeroOut", conditioning=["4", 0]),
        "6": node(
            "EmptyFlux2LatentImage" if klein else "EmptyLatentImage",
            width=WIDTH,
            height=HEIGHT,
            batch_size=1,
        ),
        ...
        "11": node(
            "SaveImage", images=["10", 0], filename_prefix="modern-models/" + model
        ),
    }
    if editing:
    ...
    if klein:
        ...
        g["7"] = node(
            "CFGGuider", model=["1", 0], positive=positive, negative=negative, cfg=1.0
        )
        ...
        g["9"] = node(
            ...
            positive=["4", 0],
            negative=["5", 0],
            latent_image=["21", 0] if editing else ["6", 0],
            seed=seed,
            steps=8,
            cfg=1.0,
            sampler_name="euler",
