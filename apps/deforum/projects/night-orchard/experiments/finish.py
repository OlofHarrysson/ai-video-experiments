"""Finish Night Orchard paintings at 1.5x speed and 24 fps, without diffusion.
...
def prepare(case, through, create, version="v001"):
    ...
    timing = {
        ...
        "fps": FPS,
        "frame_count": delivery[-1] + DELIVERY_INTERVAL,
...
def verify(root, timing, config, mode, manifest=None):
    ...
    require(
        ...
        and settings["pair_lengths"] == [DELIVERY_INTERVAL] * (len(positions) - 1)
    ...
    require(
        record["final_holds"] == (0 if moving or pair else DELIVERY_INTERVAL - 1),
        "Final hold count differs",
    )
    if moving:
        require(record["final_warps"] == DELIVERY_INTERVAL - 1, "Tail length differs")
        require(
            record["tail_recipe"]
            == {
    ...
    for row in rows:
        ...
        if index <= last and index % DELIVERY_INTERVAL == 0:
            source_index = index // DELIVERY_INTERVAL
            require(
                row["kind"] == "anchor"
            ...
            left = index // DELIVERY_INTERVAL
            require(
                row["kind"] == "interpolation"
                and row["source_pair"] == [left, left + 1]
                and row["timestep"]
                == f"{index % DELIVERY_INTERVAL}/{DELIVERY_INTERVAL}",
                "Interpolation timing differs",
            )
        elif moving:
            seconds = index * SPEED / FPS
            require(
                row["kind"] == "warp"
    ...
    save(
        target / "delivery-check.json",
        {
            ...
            "final_warps": DELIVERY_INTERVAL - 1 if moving else 0,
            "tail_changed_pixels": (
...
def tail(root, timing, config):
    ...
    record.update(
        ...
        retiming_sha256=sha(root / "retiming.json"),
        final_holds=0,
        final_warps=DELIVERY_INTERVAL - 1,
        tail_recipe={
