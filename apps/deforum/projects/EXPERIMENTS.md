# Experiment notebook index

Snapshot: 2026-09-16. **100 experiment entries across 14 projects** are listed below. This includes plans, session reports and independent reviews; it does not mean 100 completed renders. Session entries link their supporting research and review notes. Original reports own the findings, settings, media links and human feedback.

## Useful starting points

| Interest | Start here |
| --- | --- |
| Tailored doorway approach and a different world beyond it | [Through the railway doorway](modern-model-study/experiments/doorway-revision/README.md) |
| Planned, improvised and hybrid visual stories | [Three ways to tell a surreal story](modern-model-study/experiments/storytelling-lab/README.md) |
| Fluent varied pans, pullbacks and local turns | [Continuous spatial routes](modern-model-study/experiments/fluent-hour/README.md) |
| Deliberate movement and adaptive storytelling | [Living book and clockwork worlds](modern-model-study/experiments/story-hour/README.md) |
| Surreal scenes and changing worlds | [One-hour surreal lab](modern-model-study/experiments/surreal-hour/README.md) |
| Autonomous learning session and new films | [Two-hour lab: controls, RAW diagnostic, porcelain and velvet films](modern-model-study/experiments/two-hour-lab/README.md) |
| Current control of holds and transformations | [Extra paintings during the morph](modern-model-study/experiments/transition-frequency/README.md), [Earlier settling](modern-model-study/experiments/early-settle.md), [moving hold and transformation](modern-model-study/experiments/hold-transform.md) |
| Longer, changing scenes | [Dynamic journey](modern-model-study/experiments/dynamic-journey.md) |
| Prompts and transformation strength | [Prompt bridges](modern-model-study/experiments/prompt-bridges.md), [transition ramps](modern-model-study/experiments/transition-ramps.md), [CFG audition](modern-model-study/experiments/cfg-audition.md) |
| Timing and finishing | [24 fps migration](modern-model-study/experiments/timebase24.md), [faster pacing](modern-model-study/experiments/faster-pacing.md), [repaint frequency](modern-model-study/experiments/refresh-rate.md) |
| Stronger spatial motion and forward travel | [One-hour motion lab](modern-model-study/experiments/motion-hour/README.md) |
| Spatial controls | [Motion catalogue](motion-guide-study/experiments/motion-catalog.md), [three effects](motion-guide-study/experiments/motion-effects.md) |
| Models and degradation | [Opening artwork](modern-model-study/experiments/opening-art.md), [repeated repainting](modern-model-study/experiments/repaint-diagnosis.md), [preservation controls](modern-model-study/experiments/continuity-controls.md) |
| Reference Deforum appearance | [Brain Entity parameters](brain-entity-study/experiments/parameters.md), [RIFE results](brain-entity-study/experiments/rife-results.md) |

These are navigation suggestions, not a ranking of successful experiments. For project briefs and cuts, use the [project index](README.md). The [refactor plan](../../../docs/refactor-plan.md) describes how the code and documentation can become easier to navigate.

[Continuous motion hour](modern-model-study/experiments/continuous-motion-hour/README.md) tests a wider integrated spatial palette and persistent movement. Use the [motion palette](../../../docs/motion-palette.md) to select supported effects for a scene.

## All notes

Grouped by project, alphabetically within each project. New notes should be linked here; keep conclusions in their original report.

### modern-model-study — 58 notes

- [Through the railway doorway](modern-model-study/experiments/doorway-revision/README.md) — [frame review](modern-model-study/experiments/doorway-revision/quality-review.md)

- [Three ways to tell a surreal story](modern-model-study/experiments/storytelling-lab/README.md) — [independent blind review](modern-model-study/experiments/storytelling-lab/independent-review.md), [decisions](modern-model-study/experiments/storytelling-lab/DECISIONS.md)

- [Floating drift and travelling look-around previews](modern-model-study/experiments/motion-recipe-previews/README.md)

- [Continuous movement and a wider motion palette](modern-model-study/experiments/continuous-motion-hour/README.md)

- [Previous-image feedback plus optional image conditioning](modern-model-study/experiments/additive_reference.md)
- [Klein and Krea audition](modern-model-study/experiments/baseline.md)
- [Slower repaint cadence: 3, 7, 10 and 15](modern-model-study/experiments/cadence-slow.md)
- [Cadence 4 and 5 with learned interpolation](modern-model-study/experiments/cadence-spacing.md)
- [Cathedral repaint comparison](modern-model-study/experiments/cathedral-feedback.md)
- [Does extra CFG help recurrent Krea transitions?](modern-model-study/experiments/cfg-audition.md)
- [Text-only and reference-conditioned generation](modern-model-study/experiments/conditioning.md)
- [Preservation controls with the current Krea recipe](modern-model-study/experiments/continuity-controls.md)
- [Fresh versus gradually changing noise](modern-model-study/experiments/correlated-noise.md)
- [Independent painting review — 2026-09-12](modern-model-study/experiments/dynamic-journey-independent-review.md)
- [A faster journey through changing scenes](modern-model-study/experiments/dynamic-journey.md)
- [Settle promptly after the city appears](modern-model-study/experiments/early-settle.md)
- [Ease motion while repainting continues](modern-model-study/experiments/eased-motion.md)
- [Fluent varied spatial movement](modern-model-study/experiments/fluent-hour/README.md) — [independent geometry and frame review](modern-model-study/experiments/fluent-hour/motion-continuity-review.md)
- [Faster motion and morphing at 24 fps](modern-model-study/experiments/faster-pacing.md)
- [Recent latent history and gentle noise recurrence](modern-model-study/experiments/history-recurrence.md)
- [Hold a moving scene, then deliberately transform it](modern-model-study/experiments/hold-transform.md)
- [Krea: lower repaint strength](modern-model-study/experiments/krea-low-repaint.md)
- [No-motion latent feedback](modern-model-study/experiments/latent-feedback.md)
- [One hour of stronger spatial motion](modern-model-study/experiments/motion-hour/README.md)
- [Continuous movement through the Oracle transition](modern-model-study/experiments/motion-oracle.md)
- [Five independent opening-noise probes](modern-model-study/experiments/noise-openings.md)
- [Opening artwork audition](modern-model-study/experiments/opening-art.md)
- [Oracle: noise, sampling steps and more recurrent repaints](modern-model-study/experiments/oracle-steps.md)
- [Deliberate Oracle-to-owl transition](modern-model-study/experiments/owl-transition.md)
- [Persistent model volume trial](modern-model-study/experiments/persistent-model-volume.md)
- [Klein and Krea: first Pod audition](modern-model-study/experiments/pod-results.md)
- [Independent painting review — prompt bridges](modern-model-study/experiments/prompt-bridges-independent-review.md)
- [Intermediate descriptions for a changing scene](modern-model-study/experiments/prompt-bridges.md)
- [Two prompts and a wider noise range](modern-model-study/experiments/prompt-noise.md)
- [Simplified Oracle prompt](modern-model-study/experiments/prompt-oracle.md)
- [One-repaint Krea reference screen](modern-model-study/experiments/reference-screen.md)
- [More frequent, smaller repaints](modern-model-study/experiments/refresh-rate.md)
- [Does repeated repainting degrade the picture?](modern-model-study/experiments/repaint-diagnosis.md)
- [Repaint every half-second versus every second](modern-model-study/experiments/repaint-intervals.md)
- [RIFE motion-estimation scale](modern-model-study/experiments/rife-scale.md)
- [SPEED on saved Oracle transitions](modern-model-study/experiments/speed-oracle.md)
- [Three-step repainting: starting-noise sweep](modern-model-study/experiments/starting-noise.md)
- [Deliberate movement and emerging stories](modern-model-study/experiments/story-hour/README.md)
- [Saved-state replay and perturbation](modern-model-study/experiments/state-replay.md)
- [Surreal worlds: seven attempts and two favorites](modern-model-study/experiments/surreal-hour/README.md)
- [Two concepts for the $10 filmmaking session](modern-model-study/experiments/ten-dollar-concepts.md)
- [Independent painting screening: ten-dollar session](modern-model-study/experiments/ten-dollar-independent-review.md)
- [Ten-dollar independent filmmaking session](modern-model-study/experiments/ten-dollar.md)
- [Three directions: motion, prompt and interpolation](modern-model-study/experiments/three-directions.md)
- [A 24 fps working timeline with interpolation by default](modern-model-study/experiments/timebase24.md)
- [Transition ramps: independent painting review](modern-model-study/experiments/transition-ramps-independent-review.md)
- [Extra paintings during the morph](modern-model-study/experiments/transition-frequency/README.md)
- [Shaping a recurrent scene transition](modern-model-study/experiments/transition-ramps.md)
- [Describing smaller stages of the transformation](modern-model-study/experiments/transition-stages/README.md)
- [Two-hour autonomous learning session](modern-model-study/experiments/two-hour-lab/README.md)
- [Turbo schedule tail versus smaller updates](modern-model-study/experiments/turbo-schedule.md)
- [Three-step morphing: interpolation and cadence](modern-model-study/experiments/turbo-smoothing.md)
- [Ten-second transition and one/two/three Turbo intervals](modern-model-study/experiments/turbo-transitions.md)

### botanical-cathedral — 6 notes

- [Intentional 3D camera](botanical-cathedral/experiments/3d-parallax.md)
- [First SDXL feedback session — 2026-09-06](botanical-cathedral/experiments/baseline-results.md)
- [Baseline: SDXL feedback](botanical-cathedral/experiments/baseline.md)
- [Continue from a chosen frame](botanical-cathedral/experiments/continuation.md)
- [Quality at similar cost](botanical-cathedral/experiments/model-quality.md)
- [Serverless continuation and 3D practice — 2026-09-06](botanical-cathedral/experiments/serverless-results.md)

### brain-entity-study — 5 notes

- [Temporal continuity study](brain-entity-study/experiments/continuity.md)
- [Learning the feedback parameters](brain-entity-study/experiments/parameters.md)
- [Review harness practice](brain-entity-study/experiments/review-harness.md)
- [RIFE finishing comparison: P01, P03 and C02](brain-entity-study/experiments/rife-results.md)
- [Mechanical to organic opening](brain-entity-study/experiments/style.md)

### guide-redraw — 1 notes

- [Independent guide redraw](guide-redraw/experiments/independent-redraw.md)

### lantern-marsh — 2 notes

- [Baseline reference](lantern-marsh/experiments/baseline.md)
- [Overscan and a lateral camera move](lantern-marsh/experiments/overscan.md)

### marsh-story — 1 notes

- [E01 — Invitation → signal → reveal](marsh-story/experiments/story.md)

### model-comparison — 1 notes

- [Hosted stills — 2026-09-06](model-comparison/experiments/hosted-stills.md)

### motion-guide-study — 11 notes

- [Spatial effect selection](motion-guide-study/experiments/motion-catalog.md)
- [Stronger and combined spatial effects](motion-guide-study/experiments/motion-effects-2.md)
- [Three spatial effects](motion-guide-study/experiments/motion-effects.md)
- [Move-Warp guide in B's repaint loop](motion-guide-study/experiments/move-warp.md)
- [Sampling steps and accumulated input noise](motion-guide-study/experiments/noise-steps.md)
- [Repaint controls on the same short twist](motion-guide-study/experiments/repaint-controls.md)
- [Two sampler alternatives for the three-second twist](motion-guide-study/experiments/samplers.md)
- [B's repaint recipe with a small 3D camera move](motion-guide-study/experiments/seed-3d.md)
- [Same model, fixed versus changing seed](motion-guide-study/experiments/seed-comparison.md)
- [Twist, turn and ripple with cadence 3](motion-guide-study/experiments/spatial-sequence.md)
- [Guide → pixel movement → warp → repaint](motion-guide-study/experiments/walkthrough.md)

### motion-lab — 5 notes

- [E02: flat push-in](motion-lab/experiments/e02-flat-push.md)
- [E03: depth pullback](motion-lab/experiments/e03-depth-push.md)
- [E04: repaint every third frame](motion-lab/experiments/e04-sparse-repaint.md)
- [E05: stronger repainting](motion-lab/experiments/e05-stronger-repaint.md)
- [E06: approach with sparse repainting](motion-lab/experiments/e06-depth-approach.md)

### redraw-lab — 4 notes

- [Study index](redraw-lab/experiments/baseline.md)
- [E08 — Gentle 3D guide and low-denoise independent redraw](redraw-lab/experiments/e08.md)
- [E09 — Fixed noise for independent redraw](redraw-lab/experiments/e09.md)
- [E10 — Repaint only the newly uncovered regions](redraw-lab/experiments/e10.md)

### reference-studies — 2 notes

- [BonsAi motion: short baseline and 3D reveal](reference-studies/experiments/bonsai-motion.md)
- [First style-reproduction study](reference-studies/experiments/style-reproduction.md)

### sdxl-settings — 1 notes

- [Sampling effort and guidance](sdxl-settings/experiments/settings.md)

### seedream-motion — 2 notes

- [Starting reference](seedream-motion/experiments/baseline.md)
- [More visible motion from the Seedream reference](seedream-motion/experiments/motion.md)

### seedream-repaint — 1 notes

- [E07 — native Seedream frame repaint](seedream-repaint/experiments/e07-native-edit.md)
