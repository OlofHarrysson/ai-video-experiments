...
Current filmmaking test: [hold a moving scene, then transform it](experiments/hold-transform.md). Two eight-second clips compare usual and low-noise holding around the same deliberate snail-to-city transformation. Low holding noise is the assistant candidate for preserving local detail; human playback judgment is pending.
...
Latest: [Repaint-frequency comparison](experiments/refresh-rate.md) is complete at Olof's requested CFG1: 2/s baseline, 4/s same noise, and 4/s with the whole noise curve reduced 15%. Timed prompts/motion and shared-timestamp seeds stay fixed; the new baseline reproduces all thirteen prior CFG1 paintings pixel-for-pixel. Four/s at original noise opens a wider city; lower noise leaves a large solid shell/disk and weakens conversion. Additional painted stages do not remove all doubled interpolation edges. Local reviewer defaults to 2/s versus 4/s original noise, with all three selectable; human preference pending, keep 2/s as the accepted baseline. All 62 jobs/1,018 archive files and three 156-frame, 24 fps deliveries verified. Owned RTX 4090 and scratch/media removed, model cache retained. About $0.25 estimated including disk, cumulative session about $2.52 of $10.

Earlier: [CFG audition](experiments/cfg-audition.md) is complete: 0.8/1.0/1.3/1.6 at identical staged prompts, noise ramp, recurrent initialization, motion and seeds. Higher CFG visibly opens the shell into architecture earlier and adds city detail, but increases repaint changes; RIFE still shows doubled edges in the inspected 2.5–3s transition. Default local reviewer compares 1.0 with 1.3, with all four selectable. Assistant favors 1.3 as a detailed alternative, not an established smoothness fix; preserve CFG1 pending human feedback. All 48 jobs/799 archive files and four 156-frame, 24 fps deliveries are verified locally. Owned RTX 4090 and temporary media/session removed; model cache retained. Estimated cost $0.28 including disk allowance, within the original $10 budget. Off-1 warm execution was about 8s versus 4.6s at CFG1.

Earlier: [intermediate scene descriptions](experiments/prompt-bridges.md) compares direct city text with shell-with-buildings → hollow architectural shell → city at the same noise and motion settings. Staged text supplies a clearer hybrid at 2–2.5s, but the principal repaint jump remains. Keep the existing direct-prompt approach pending playback feedback. Both 6.5-second 24 fps/RIFE clips and all 24 new paintings are preserved; owned Pod/scratch/media removed, model cache retained. This round is about $0.13 estimated. The runner now supports prompt events without restarting the noise ramp.

Earlier: [six transition-ramp tests](experiments/transition-ramps.md) compare starting noise, peak, number of ramp paintings and rise/return shape. All six 6.5-second videos are local and verified. Lower starts mainly delay conversion; the same smooth curve peaking at 0.78 opens a wider city than 0.70, with a larger change between paintings. Those two clips are the assistant shortlist; Olof's preference is pending. The liked dynamic film remains the reference. All 72 new jobs/1,192 archive files are preserved, owned A100 deleted and model cache retained; posted cost for that round is $0.5873, checked during the prompt-bridge follow-up. [CFG and other transition controls](../../../../docs/research/krea-transition-controls.md) explains why Turbo stays at ComfyUI CFG 1 and recommends fixed intermediate scene descriptions as the next hypothesis.
