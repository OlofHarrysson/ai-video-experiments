...
The subsequently approved [correlated-noise comparison](correlated-noise.md) isolates unchanged-prompt stability in a stationary recurrent Krea scene at fixed sigma and sampling intervals. Actual tensors and clearly labeled latent diagnostics are preserved. Correlation 0.85 causes strong color drift; retain independent noise. Historical fixed-seed SDXL results do not settle other correlation settings with Krea. Motion and intentional prompt transitions remain untested with this intervention. See [noise controls and evidence limits](../../../../../docs/research/krea-transition-controls.md).
