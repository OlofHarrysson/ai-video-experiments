...
Both six-second finished clips are complete. **The one-second interval is the assistant's calmer audition candidate for Olof's stated aim of less frequent change.** The half-second branch is the more active alternative: later frames show more rebuilding of the little towers, arches and central flame. Both retain the warm cream/teal/orange palette and recognizable spiral composition. These are first-pass findings from selected frames; Olof has not selected a winner in playback.
...
[Generation runner](repaint_intervals.py) · [verification and assembly](repaint_intervals_review.py). Private session receipts: `apps/deforum/work/repaint-interval-session/`. The default three-step model recipe and interpolation settings remain unchanged; this experiment has not established a replacement for Olof's accepted baseline without his playback judgment.
