...
def encode(root, clock=None):
    ...
    if not target.exists():a.editing.encode(root/'frames',target,fps=clock.fps)
    ...
    assert int(p['nb_read_frames'])==clock.count and p['avg_frame_rate']==f'{clock.fps}/1'
    assert float(p['duration'])==float(clock.duration_seconds)
    return p
...
def base(level, study=study):
    return study.branch(level)/f'cadence-{study.timing().cadence}'


def build_raw(study=study):
    """Reconstruct diagnostic frames from anchors, sharing time-only maps across cases."""
    clock=study.timing()
    for local in range(clock.count):
        ...
        for level in study.LEVELS:
            root=base(level,study);target=root/f'frames/{local:04d}.png'
        ...
        if local%24==0:print('Raw frames',local,f'/{clock.count}',flush=True)
    for level in study.LEVELS:
        root=base(level,study)
        a.save(root/'frame-hashes.json',{p.name:a.sha(p) for p in sorted((root/'frames').glob('*.png'))})
        encode(root,clock)


def prepare(level, study=study):
    root=base(level,study);clock=study.timing();rows=[]
    ...
    for i,f in enumerate(range(clock.start,clock.start+clock.count,clock.cadence)):
        ...
        assert_warp_matches(t.warp_at_time(np.asarray(Image.open(parent).convert('RGB')),clock.seconds(f-clock.cadence),clock.seconds(f)),before)
    ...
    hashes=json.loads((root/'frame-hashes.json').read_text());assert len(hashes)==clock.count
    for name,digest in hashes.items():assert a.sha(root/'frames'/name)==digest
    for local in (0,1,clock.cadence-1,clock.cadence,clock.cadence+1,clock.count-clock.cadence-1,clock.count-clock.cadence,clock.count-1):
        f=clock.start+local;anchor=clock.start+local//clock.cadence*clock.cadence
    ...
    a.save(root/'verification.json',{'verified':True,'level':level,'repaints':rows,'probe':encode(root,clock),
    ...
    canvas=Image.new('RGB',(1152,284*((clock.count//clock.cadence+2)//3)),'#15191d');draw=ImageDraw.Draw(canvas)
    for i,f in enumerate(range(clock.start,clock.start+clock.count,clock.cadence)):
        x=(i%3)*384;y=(i//3)*284
...
def finish(level, study=study):
    root=base(level,study);raw=root/'rife-raw';out=root/'interpolated';clock=study.timing()
    ...
    for i in range(clock.count):
        source=raw/f'frames/{i:04d}.png' if i<clock.count-clock.cadence else root/f'frames/{i:04d}.png'
        a.copy(source,out/f'frames/{i:04d}.png')
        rows.append({'frame':i,'seconds':i/clock.fps,'source':str(source.relative_to(a.PROJECT)),'sha256':a.sha(source)})
    for i,f in enumerate(range(clock.start,clock.start+clock.count,clock.cadence)):
        np.testing.assert_array_equal(np.asarray(Image.open(out/f'frames/{i*clock.cadence:04d}.png')),np.asarray(Image.open(root/f'anchors/{f:04d}.png')))
    p=encode(out,clock);assert (p['width'],p['height'])==(1536,1024)
    subprocess.run(['ffmpeg','-v','error','-xerror','-i',str(out/'preview.mp4'),'-f','null','-'],check=True)
    record={'frames':rows,'probe':p,'anchors_pixel_identical':True,'final_warp_starts_seconds':float(clock.duration_seconds-clock.repaint_seconds),
           'repaint_seconds':1,'rife_receipt_sha256':a.sha(raw/'manifest.json'),'feedback_generation_changed':False}
    manifest=out/'manifest.json'
    if manifest.exists():
        assert json.loads(manifest.read_text())==record, 'Existing finishing receipt differs'
    else:
        a.save(manifest,record)
    print('FINISHED',level,flush=True)


def compare(levels,name, study=study, labels=LABELS, caption_at=None):
    out=study.OUT/name;columns=2;rows=(len(levels)+1)//2;clock=study.timing()
    ...
    for i in range(clock.count):
        ...
        for index,level in enumerate(levels):
            ...
            with Image.open(base(level,study)/f'interpolated/frames/{i:04d}.png') as im:canvas.paste(im.resize((768,512),Image.Resampling.LANCZOS),(x,y+42))
            draw.text((x+12,y+9),labels[level]+(' | '+caption_at(i/clock.fps) if caption_at else ''),font=font,fill='white')
        a.image(out/f'frames/{i:04d}.png',np.asarray(canvas))
    a.save(out/'manifest.json',{'probe':encode(out,clock),'levels':levels,'frame_hashes':{p.name:a.sha(p) for p in sorted((out/'frames').glob('*.png'))}})
