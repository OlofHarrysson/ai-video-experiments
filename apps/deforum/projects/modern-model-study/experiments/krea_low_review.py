"""Verify the low-repaint archive and render a matched comparison."""
import copy
import json
import subprocess
import numpy as np
from PIL import Image, ImageDraw, ImageFont
import krea_low_repaint as c
...
def main():
    ...
    sources = [(c.OUT / key, 'Repaint ' + str(value)) for key, value in c.STRENGTHS.items()]
    sources.append((c.c.OUT/'krea/gentle', 'Earlier repaint 0.30'))
    diagnostics = []
    for root, label in sources:
        changes = []
        for f in range(3, c.FRAMES, 3):
            before = np.asarray(Image.open(root/f'warped-inputs/{f:04d}.png').convert('RGB'), dtype=np.float32)
            after = np.asarray(Image.open(root/f'anchors/{f:04d}.png').convert('RGB'), dtype=np.float32)
            changes.append(float(np.mean(np.abs(after-before))))
        diagnostics.append({'label': label, 'mean_absolute_repaint_change_0_255': float(np.mean(changes)),
            'per_anchor': changes})
    c.a.save(c.OUT/'repaint-diagnostics.json', {'interpretation': 'pixel change after spatial warp; includes texture, lighting and structure; not a perceptual flicker score', 'results': diagnostics})
    font = ImageFont.truetype('/System/Library/Fonts/Helvetica.ttc', 24)
    for pair, indices in [('all', range(4)), ('lower-vs-control', [1,3])]:
        target = c.OUT / 'comparisons' / pair / 'preview.mp4'
        if target.exists(): continue
        for f in range(c.FRAMES):
            canvas = Image.new('RGB', (1536,1104 if pair=='all' else 552), '#15191d')
            draw = ImageDraw.Draw(canvas)
            for i,j in enumerate(indices):
                root,label = sources[j]; x,y=(i%2)*768,(i//2)*552
                with Image.open(root/f'frames/{f:04d}.png') as im:
                    canvas.paste(im.convert('RGB').resize((768,512), Image.Resampling.LANCZOS), (x,y+40))
                draw.text((x+16,y+8), label, font=font, fill='white')
            c.a.image(target.parent/f'frames/{f:04d}.png', np.asarray(canvas))
        c.a.editing.encode(target.parent/'frames',target,fps=c.FPS)
        c.a.save(target.parent/'manifest.json',{'sources':[str(sources[j][0].relative_to(c.a.PROJECT)) for j in indices],
            'source_manifest_hashes':[c.a.sha(sources[j][0]/'manifest.json') for j in indices],
            'method':'synchronized 12 fps source frames, no interpolation'})
    print('Verified',len(rows),'feedback anchors and all three clips')
