"""Matched RGB-feedback twist with coarse versus subdivided Turbo schedules."""
import argparse
import json
...
a.transport.DEPLOYMENT = a.APP / 'work/turbo-schedule-session/deployment.json'
...
def graph(strength, seed):
    g = schedules.repaint_graph(strength, seed)
    g['11']['inputs']['filename_prefix'] = 'turbo-schedule-motion/' + strength
    return g
...
def run(strength):
    ...
    for f in range(CADENCE, FRAMES, CADENCE):
        ...
        name = f'turbo-schedule-motion-v001-{model}-{strength}-{f:04d}'
        ...
        if matches:
            ...
            folder = a.transport.submit(a.PROJECT, name, g, 1, source=source,
                lineage={'source_sha256': a.sha(source), 'parent_sha256': a.sha(prior),
                         'study': 'turbo-schedule-motion-v001', 'frame': f})
