...
def sources():
    ...
    for page in range(2):
        ...
        rows = [
            [
                (
                    Image.open(root / f"anchors/{f:04d}.png"),
                    f"{label} | frame {f} | {f / 24:.1f}s",
                )
                for label, root in [("Previous", old), ("Earlier settling", new)]
            ]
            for f in frames
        ]
    ...
    rows = [
        [
            (
                Image.open(root / f"anchors/{f:04d}.png").crop((768, 256, 1408, 682)),
                f"{label} | frame {f} | native detail",
            )
            for label, root in [("Previous", old), ("Earlier settling", new)]
        ]
        for f in frames
...
def delivery():
    ...
    for start in (103, 115):
        ...
        rows = [
            [
                (
                    Image.open(root / f"frames/{f:04d}.png").crop(
                        (768, 256, 1408, 682)
                    ),
                    f"{label} | frame {f} | {f / 24:.3f}s",
                )
                for label, root in [("Previous", old), ("Earlier settling", new)]
            ]
            for f in frames
    ...
    header = e.OUT / "comparison-header.png"
    im = Image.new("RGB", (1536, 30), "#181818")
    d = ImageDraw.Draw(im)
    font = ImageFont.load_default(size=20)
    d.text((12, 4), "PREVIOUS - SETTLES AT 6.5s", fill="white", font=font)
    d.text((780, 4), "NEW - STARTS SETTLING AT 4.5s", fill="white", font=font)
    im.save(header)
    ...
    (e.OUT / "review/comparison-check.json").write_text(
        json.dumps({"verified": True, "video": info, "sha256": sha(video)}, indent=2)
    )


if __name__ == "__main__":
    p = argparse.ArgumentParser()
    p.add_argument("stage", choices=["sources", "delivery"])
    a = p.parse_args()
