...
Olof prioritizes generating the opening and subsequent frames with the same model before tuning denoise. He also proposes fixed seed as a possible continuity requirement. [Original-source inspection](../../../../../docs/research/deforum-architecture-comparison.md#starting-model-and-seed-continuity) confirms that one loaded model can generate the entire sequence, while fixed and changing seeds are both supported. Neither same-model provenance nor a fixed seed alone guarantees good animation.

Proposed, not executed: generate a fresh opening with the installed P3 art recipe (SDXL base plus its art LoRA), then use that exact checkpoint, VAE, encoders, LoRA weights, prompt and resolution for repainting. Hold the seed fixed, including for the opening. First inspect matched no-warp and adjacent-frame-warp repaints at the existing 0.58 denoise, so the initial question is whether this internally consistent recipe can preserve its own opening. This is a diagnostic, not a clean causal comparison against the different Seedream scene. If necessary, test gentler denoise afterward. Do not use the Seedream marsh as the input to this baseline; preserve it as a visual reference.

Once a single step preserves the artwork sufficiently, assess a short feedback sequence for accumulated drift, blur, freezing and gradual change. Fixed seed is an experimental control, not a claimed optimum: the liked P3 animation used incrementing seeds and a different denoise. Surface at most two meaningful video alternatives after assistant review. Preserve the planned move into intentional 3D camera work after this bounded calibration.
