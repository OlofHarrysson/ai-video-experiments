...
**Next input-mode tests (2026-09-08, explicit):** Olof requests reassembly without cadence blending first, then text-only and text-plus-image experiments with both models. He suspects Klein's accumulating saturation may be excessive guidance or diffusion strength, but explicitly expresses uncertainty. Both prior graphs used CFG 1; the cause is unresolved. He questions img2img as the animation mechanism. Explain conventional partial img2img, native reference conditioning and text-only generation separately; do not infer understanding from presenting that distinction.
