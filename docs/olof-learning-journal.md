...
**Krea regional-composition feedback (2026-09-10, explicit):** Olof likes the Krea stills but questions whether regional prompting adds value over a complete prompt for two distinct objects placed left/right. He approves an eight-image test with three robots, fixed color/prop identities, two layouts and unequal sizes. This establishes a preference for testing practical composition control on a harder scene; it does not establish a preferred Krea conditioning recipe or approval of the new outputs. See the [layout benchmark](../apps/comfyui/projects/regional-composition/experiments/krea-layouts.md).
