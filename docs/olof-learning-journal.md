...
Completed on 2026-09-07: two five-second videos from the preferred Seedream image with a stronger shared 3D camera path: recurring SDXL feedback and independent guide redraw. Give each its own large preview in chat. Longer equal-duration clips should be easier to compare than the previous one-second redraw sample.
...
The [filmmaking research guide](research/filmmaking-for-ai-animation.md) was created after rendering. Its availability is not evidence it was read or understood. A proposed next exercise is a short visual reveal planned in three images; no new film or understanding milestone is implied by that proposal.
