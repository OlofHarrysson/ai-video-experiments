...
The [completed latent comparison](../../apps/deforum/projects/modern-model-study/experiments/latent-feedback.md) reproduces flattening without repeated VAE encoding. Both first repaints exactly match the RGB controls. The next proposed comparison targets the sampler schedule: the verified eight-step full schedule ends with sigma 0.311 then zero, whereas our 0.10 recipe starts at 0.260 and makes eight updates below that. Compare schedule construction with a matched starting noise coefficient before attributing the result to distillation or building latent-space motion. This comparison is now complete, as described below.


## Matched Turbo schedule result

The [completed study](../../apps/deforum/projects/modern-model-study/experiments/turbo-schedule.md) matches starting sigma 0.3109010756, seeds, model and prompt. One final interval of the standard eight-step schedule retains more broad architectural shading than eight subdivided intervals. This difference appears both after 24 no-motion latent-feedback cycles and in matched three-second Lanczos/RGB-feedback animations. The single interval keeps the flame cleaner; the subdivided schedule introduces more mottled surface patterns. Both still simplify and lose some richness. This supports a practical recipe choice, not a general explanation of distillation.

The comparison changes intermediate noise levels and evaluation count together. It cannot tell which component matters individually, does not compare distilled and undistilled weights, and does not demonstrate long-shot quality. The single interval is an assistant-selected candidate awaiting human playback feedback. Median warm motion request execution was 1.833 versus 9.360 seconds; end-to-end timings also include setup, warping and transfer. Keep Lanczos and the feedback loop while testing a somewhat longer creative shot before broadening the model search.


## Longer shot and native tail length

The [ten-second transition](../../apps/deforum/projects/modern-model-study/experiments/turbo-transitions.md) confirms that the single final interval still loses substantial shading across 39 RGB-feedback repaints. Its spatial twist and expansion remain visible. Branching from the same generated frame, the final two intervals restore some contrast and the final three restore stronger shading and more elaborate architecture, with larger changes across repaint boundaries. These are short, eleven-repaint comparisons; three intervals have not yet demonstrated ten-second quality.

Starting sigma changes from 0.311 to 0.513 to 0.655 across these tails. This tests native-tail recipes with different noise strength, unlike the earlier matched-starting-noise comparison. It supports a practical tradeoff between preserving existing forms and rebuilding richer artwork; it does not isolate step count, prove a distillation mechanism, or establish a guaranteed stable animation recipe.
