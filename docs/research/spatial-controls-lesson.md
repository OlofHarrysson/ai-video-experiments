...
Next step: the [spatial effects browser](../../apps/deforum/projects/motion-guide-study/experiments/motion-catalog.md) expands the drawing-based approach to 22 direct controls on one image, with frame zero serving as the original. Olof selected direct effects now and deferred original guide/depth mechanisms.
