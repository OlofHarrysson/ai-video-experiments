...
Production tooling serves the assistant's filmmaking work: plan motion, inspect previews and joins, coordinate prompts/noise with the story, backtrack and regenerate as needed. The assistant operates these tools and brings Olof the strongest resulting videos. Additional human authoring controls require a specific user need; a request to improve the assistant's workflow does not imply that Olof wants to operate a planner. The existing human-facing media reviewer remains useful for playback, comparisons and taste feedback.
