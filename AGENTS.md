...
- Current research direction: Olof identifies Safety Marc's presets as central evidence for successful spatial image controls. [Spatial-control study](docs/research/bonsai-spatial-motion.md) inventories all 95 files and maps camera schedules, guide flow and guide compositing. Proposed next mechanism is Move-Warp's actual wave guide in B's feedback recipe; implementation awaits alignment. The earlier zoom/3D sequence remains in the [experiment brief](apps/deforum/projects/reference-studies/experiments/bonsai-motion.md).
- Stay with ComfyUI; Olof deferred A1111/Forge. The [motion-guide walkthrough](apps/deforum/projects/motion-guide-study/README.md) now shows actual guide pairs, estimated arrows, local pixel warps and six matched ComfyUI repaint samples. The full guide-flow feedback loop remains unimplemented; the subsequent B-recipe 3D test is separate from a completed preset reproduction.
