...
- Runnable path: `uv run python experiment.py init-project NAME`, then `run --project NAME --experiment EXPERIMENT ...` in `apps/deforum/`. Write the experiment note before rendering. `setup-pod.sh` installs pinned nodes and SDXL. Use the CLI for infrastructure until MCP tool execution is verified.
...
- Next experiments: continue from a selected frame, prove depth-based parallax, then benchmark a modern model at comparable cost. These are proposals, not implemented workflows. Current 3D mode requires depth, reuses it through the loop, and does not establish long-shot scene consistency.
