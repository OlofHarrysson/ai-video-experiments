...
- Before further denoise tuning, generate the opening and its continuation with one exact model recipe (checkpoint, VAE and LoRA weights), holding prompt and seed fixed for the diagnostic. Olof rejected the walkthrough's abrupt repaint and prioritized removing the Seedream-to-SDXL mismatch. Fixed seed is a test control, not a universal Deforum requirement; see [source findings](docs/research/deforum-architecture-comparison.md#starting-model-and-seed-continuity).
