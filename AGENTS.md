...
- Next two tests: an 8–10-second reference-style zoom/morph, then promptly a depth-based sideways move with rotation and a near foreground layer. Olof accepted the short baseline but wants 3D transformations prioritized over further zoom-only variations. [Experiment brief](apps/deforum/projects/reference-studies/experiments/bonsai-motion.md).
...
- [Same-model seed comparison](apps/deforum/projects/motion-guide-study/experiments/seed-comparison.md) complete: Olof rejected A's fixed seed and selected B's changing seed. Keep incrementing seeds for simple reproducibility. The [small 3D follow-up](apps/deforum/projects/motion-guide-study/experiments/seed-3d.md) now has a depth-only preview and six-second repaint: parallax is verified in the guide; the repaint develops an orb/planet and fills exposed gaps, but camera motion remains subtle and depth is still reused from the opening. Olof's feedback on the 3D result is pending. Media is local and owned cloud resources cleaned up.
